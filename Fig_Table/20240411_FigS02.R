...
sim.filename = sprintf("%s_FigS02.pdf", format(Sys.Date(), "%Y%m%d"))
...
for (i in 1:4) {
  ...
  axis(4, at = seq(0, 2, 0.5), col = "#17becf", col.axis = "#17becf")
  
  Re.i = aggregate(RRe ~ time, data = pmcmc.sim, FUN = CI95.fx)
  Re.i = do.call(data.frame, Re.i)
  colnames(Re.i) = gsub("RRe[.]", "", colnames(Re.i))
  Re.i = merge(x = Re.i, y = pomp.data, all.x = T)
  
  with(
    Re.i, {
      lines(mean ~ dec.date, data = Re.i, col = alpha("#17becf", 0.5))
      polygon.fx(x = dec.date, LB = LB, UB = UB, col = alpha("#17becf", 0.1))
    }
  )
  
  abline(h = 1, col = alpha("#1f77b4", 0.5), lty = 3)
  ...
  if (i == 3) {
    text(
      x = 2025.2, y = 2.5, labels = "Effective Reproduction Number", 
      col = "#17becf", cex = 1.25, srt = 270, xpd = NA, font = 2
    )
